...
const contactLinks = [
  { label: 'Phone', value: '0420 666 397', href: 'tel:+61420666397' },
  { label: 'Email', value: 'liamlsat93@gmail.com', href: 'mailto:liamlsat93@gmail.com' },
]
...
function ContactLink({ label, value, href, large = false }) {
  ...
  const hoverDisplay = large ? value : value

  return (
    <a className={`kinetic-link ${large ? 'kinetic-link-large' : ''}`} href={href} data-label={`→ ${hoverDisplay}`}>
      <span>→ {display}</span>
    </a>
  )
}
